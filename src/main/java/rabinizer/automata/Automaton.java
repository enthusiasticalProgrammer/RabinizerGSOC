...
public abstract class Automaton<State> /* implements AccAutomatonInterface */ {
    ...
    /*
     * protected String nameOfState(State s) { return stateLabels.get(s); }
     */
    ...
    public Automaton<State> generate() {
        ...
        while (!workstack.empty()) {
            ...
            // if (succValSets.isEmpty()) { // TODO empty or true and
            // non-progress or just selfloops?
            // sinks.add(curr);
            // } else {
            transitions.put(curr, new HashMap<>());
            for (ValuationSet succVals : succValSets) {
                // Tuple<State, String> succStateLabel = generateSuccState(curr,
                // succVals);
    ...
    public Automaton<State> useSinks() {
        for (State s : states) {
            ...
            if (edgeBetween.containsKey(selfloop) && edgeBetween.get(selfloop).isAllVals() && !s.equals(initialState)) {
                // (transitions.get(s).size()==1) &&
                // (transitions.get(s).values().contains(s))
                sinks.add(s);
    ...
    public String toDotty() {
        ...
        for (State s : states) {
            /*
             * if (finalStates.contains(s)) { r +=
             * "node [shape=Msquare, label=\"" + displayLabels.get(s) + "\"]\""
             * + displayLabels.get(s) + "\";\n"; } else
             */
            if (s == initialState) {
        ...
        for (Map.Entry<State, HashMap<ValuationSet, State>> stateHashMapEntry : transitions.entrySet()) {
            for (Map.Entry<ValuationSet, State> edge : stateHashMapEntry.getValue().entrySet()) {
                r += "\"" + stateHashMapEntry.getKey() + "\" -> \"" + edge.getValue() + "\" [label=\"" + edge.getKey()
                        + "\"];\n";
            }
        }
        return r + "}";
    ...
    public String toHOA() {
        ...
        // Map<Tuple<Formula, KState>, Tuple<Formula, KState>> normalStates =
        // new HashMap<Tuple<Formula, KState>, Tuple<Formula, KState>>();
        for (State s : states) {
