...
public class MyBDD {
    ...
    public BDD bdd;
    public boolean valuationType; // true for valuations, false for general
                                  // formulae
    ...
    public String BDDtoString() { // FIXME!!! uzavorkovani // instead
                                  // toNumerical and map variableToString
        if (bdd.isOne()) {
            return "tt";
        } else if (bdd.isZero()) {
            return "ff";
            ...
            if (bdd.high().isOne()) {
                ...
                result += "(" + variableToString(bdd.level()) + "&" + new MyBDD(bdd.high(), valuationType).BDDtoString()
                        + ")";
                firstOp = true;
            }
        ...
        while (true) {
            if (other.bdd.isOne()) {
                ...
                if (other.bdd.high().isOne()) {
                    ...
                    high = FormulaFactory.mkAnd(other.variableToFormula(bdd.level()),
                            new MyBDD(other.bdd.high(), other.valuationType).BDDtoFormula());
                }
