...
public class Simplify_Aggressively_Visitor implements Formula_Visitor {
	
	private static Simplify_Aggressively_Visitor instance = new Simplify_Aggressively_Visitor();
	
	private Simplify_Aggressively_Visitor(){
		super();
	}
	
	public static Simplify_Aggressively_Visitor getVisitor(){
		return instance;
	}
		...
		for (int i = list.size()-1; i >= 0; i--) {
			for (int j = list.size() - 1; j > i; j--) {
				if(list.get(i).get_id()==list.get(j).get_id()){
					...
					Implication_Visitor imp= Implication_Visitor.getVisitor();
					if(list.get(i).acceptBinarybool(imp,list.get(j))){
						...
						return FormulaFactory.mkConst(false);
					}else if(list.get(j).acceptBinarybool(imp,FormulaFactory.mkNot(list.get(i)).acceptFormula(this))){
						return FormulaFactory.mkConst(false);
					}else{
			...
			for(int j=list.size()-1;j>i;j--){
				if(list.get(i).get_id()==list.get(j).get_id()){
					...
					if(list.get(i).acceptBinarybool(imp,list.get(j))){
						...
						list.remove(j);
					}else if(FormulaFactory.mkNot((list.get(j)).acceptFormula(this)).acceptBinarybool(imp,list.get(i))){
						return FormulaFactory.mkConst(true);
					}else if(FormulaFactory.mkNot((list.get(i)).acceptFormula(this)).acceptBinarybool(imp,list.get(j))){
						return FormulaFactory.mkConst(true);
					}else{
			...
			return (FormulaFactory.mkOr(FormulaFactory.mkU(FormulaFactory.mkNot(((UOperator) child).right)
					,FormulaFactory.mkAnd(FormulaFactory.mkNot(((UOperator) child).left),FormulaFactory.mkNot(
							((UOperator) child).right))),FormulaFactory.mkG(
									FormulaFactory.mkNot(((UOperator) child).right)))).acceptFormula(this);
	...
	public Formula visitU(UOperator u) {
		if(u.right.acceptBool(Suspendable.getVisitor())||u.right.acceptBool(Eventual_Visitor.getVisitor())){
			...
			if(l.acceptBinarybool(imp, r)){
				...
				return FormulaFactory.mkOr(r,FormulaFactory.mkF(FormulaFactory.mkAnd(
						FormulaFactory.mkX(r),l))).acceptFormula(this);
